import { Paper, Select, TextField, Button, IconButton, Box, FormControl, InputLabel, List, ListItem, ListItemText, MenuItem, Typography } from "@mui/material";
          ...
          <List>
            {escalas.map((escala) => (
              <ListItem
                key={escala.id}
                secondaryAction={
                  <IconButton edge="end" onClick={() => removerEscala(escala.id)}>
                    <Trash />
                  </IconButton>
                }
              >
                <ListItemText
                  primary={
                    `${nomeDiaSemana(escala.diaSemana)} - ${escala.horaInicio} até ${escala.horaFim}`
                  }
                />
              </ListItem>
            ))}
